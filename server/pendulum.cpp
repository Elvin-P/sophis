...
std::array<double, 4> Pendulum::simulate(double u, std::array<double, 4> x0, double ts = 0.05) {
  u = u * 2 * maxu - maxu;
...
double Pendulum::rewardFunc(std::array<double, 4> x, double u) {
  ...
  x[2] = Pendulum::normalizeAngle(x[2]);
...
double Pendulum::getNormalInput(double uc_left, double uc_right, double ud) {
...
double Pendulum::normalizeAngle(double a) {
